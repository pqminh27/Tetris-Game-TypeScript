...
class GameManager {
    ...
    checkRows() {
        const rows = this.currentShape.blocks
        ...
        rows.forEach((row) => {
            ...
            this.pile.forEach((shape) => {
                shape.blocks.forEach((block) => {
                    if (block.position.y === row) counter += this.blockSize
                })
            })
    ...
    clearRow(row: number) {
        this.pile.forEach((shape) => {
            let newBlocks = shape.blocks.filter(
                (block) => block.position.y !== row
            )
            newBlocks.forEach((block) => {
                if (block.position.y < row) block.relativeY++
            })
